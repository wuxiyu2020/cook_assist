...
elif [[ "${board}" == "asr5502" ]]; then
	./tools/asr5502.sh  $type $app ${board} ${REGION} ${ENV} ${CONFIG_DEBUG} "${ARGS}"
...
# after_build.sh脚本用来在开发环境做些编译后的动作，根据自己需求自行创建编辑，它放在.gitignore里面了，不会上传到仓库
if [[ -f after_build.sh ]]; then
	./after_build.sh
fi

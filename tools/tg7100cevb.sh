...
m_path_tmp=$PATH
export PATH=$(pwd)/Living_SDK/build/compiler/riscv64_unkown_elf_gcc8.3.0/Linux64/bin:$PATH
...
{
	rm -rf tools/tg7100c/img/$1@$2.bin
}
...
function files_cp()
{
	...
	#cp Living_SDK/platform/mcu/tg7100c/network/ble/ble.a prebuild/lib/
	cp Living_SDK/platform/mcu/tg7100c/gcc/flash_rom.ld prebuild/lib/
	cp Products/example/mars_template/mars_devfunc/irt102m/Lib_Irt102mDataCalibra.a prebuild/lib/
	...
	cp --parents Living_SDK/framework/mars_httpc/include/*.h prebuild/include
...
function build_bin()
{
  ...
  cd ./tools/tg7100c/
  ./tg_iot_tool --chipname=tg7100c --xtal=40M --firmware="./../../out/$2@$3/$2@$3.bin" --build
  cd -
  cp ./tools/tg7100c/chips/tg7100c/ota/FW_OTA.bin.xz.hash $OUT/$2@$3_ota.bin

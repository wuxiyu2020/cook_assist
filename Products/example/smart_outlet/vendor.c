...
#define RELAY_GPIO  5  //12 for tg7100CA
#define KEY_GPIO    3
...
void product_init_switch(void)
{
    ...
    app_info("get kv save switch state:%d, ret:%d", state, ret);
    if (ret != 0)
        state = ON;
...
static void timer_period_save_device_status_cb(void *arg1, void *arg2)
{
   ...
   if(ret != 0 || state != switch_state) {
        app_info("KV DEVICE_STATUS Update!!!,ret=%d.\n",ret);
        ret = aos_kv_set(KV_KEY_SWITCH_STATE, &switch_state, len, 0);
        if (ret < 0) app_info("KV set Error: %d\r\n", __LINE__);
    }
}
#endif
...
void product_set_switch(bool state)
{
    ...
    app_info("product_set_switch, state:%d, switch_state:%d", state, switch_state);
...
int set_device_meta_info(void)
{
    ...
    if ((strlen(product_key) > 0) && (strlen(product_secret) > 0) \
        ...
        app_crit("PK:%s", product_key);
        app_crit("DN:%s", device_name);
        return 0;
    } else {
#if (defined (TG7100CEVB))
	...
	extern int ali_factory_media_get(char **pp_product_key, char **pp_product_secret,
        ...
        int res = ali_factory_media_get(
                    ...
                    &productidStr);
        if (0 != res) {
            app_err("ali_factory_media_get res = %d\r\n", res);
            return -1;
        } else {
            ...
            app_crit("PK:%s", p_product_key);
            app_crit("DN:%s", p_device_name);

            return 0;
        }
#endif
        app_err("no valid device meta data");
        return -1;
    }
}

...
#warning "Unsupported awss mode!!!"
    ...
    dump_device_state(STATE_CODE_WIFI_AWSS, "start awss type:%d", awss_type);
#endif
...
{
    app_info("awss timeout, stop awss");
...
{
    app_info("connect ap failed timeout");
    ...
    while (1) {
        if (!product_get_key()) {
            ...
            if ((awss_flag == 0) && (nCount >= AWSS_REBOOT_CNT)) {
                app_info("do awss reboot");
                do_awss_reboot();
                break;
            } else if ((awss_flag == 1) && (nCount > AWSS_RESET_CNT)) {
                app_info("do awss reset");
                do_awss_reset();
                break;
            }
            nCount = 0;
        ...
        switch (cur_state) {
            ...
            case AWSS_NOT_START:
                if (pre_state != cur_state) {
                    ...
                    app_err("[net_state]awss timeout, set led %d", switch_stat);
            ...
            case CONNECT_AP_FAILED_TIMEOUT:
                if (pre_state != cur_state) {
                    switch_stat = (int)product_get_switch();
                    app_info("[net_state]connect AP failed timeout, set led %d", switch_stat);
                    product_set_led(switch_stat);
                }
                break;
            ...
            case CONNECT_AP_FAILED:
                ...
                if (pre_state != cur_state) {
                    app_err("[net_state]connect AP failed");
            ...
            case CONNECT_CLOUD_SUCCESS:
                if (pre_state != cur_state) {
                    ...
                    switch_stat = (int)product_get_switch();
                    app_info("[net_state]connect cloud success, set led %d", switch_stat);
                    product_set_led(switch_stat);
                }
                break;
            ...
            case FACTORY_BEGIN:
                app_info("[net_state]factory begin");
                set_net_state(FACTORY_SUCCESS);
                break;

            case FACTORY_SUCCESS:
                if (pre_state != cur_state) {
                    app_info("[net_state]factory success, set led OFF");
                    product_set_led(OFF);
                }
                break;
...
void check_factory_mode(void)
{
    ...
    if (strlen(ap_config.ssid) <= 0) {
        app_info("scan factory ap, set led ON");
        product_set_led(ON);

        ret = scan_factory_ap();
        if (0 != ret) {
            set_net_state(UNCONFIGED);
            app_info("not enter factory mode, start awss");
            app_start_wifi_awss(0);
        }
    } else {
        if (awss_flag == 1) {
            app_info("start awss with netconfig exist");
            set_net_state(RECONFIGED);
            aos_kv_del("awss_flag");
            app_start_wifi_awss(awss_flag);
        } else {
            set_net_state(GOT_AP_SSID);
            app_info("start connect");

...
void user_post_property(property_report_msg_t * msg)
{
    ...
    if (0 == res) {
        ...
        app_info("Post Property Message ID: %d Payload %s", res, property_formated);
        ...
        app_info("Post Property Message ID: %d Payload %s", res, property_payload);
    }
...
void report_device_property(char *seq, int flag)
{
    ...
    if (0 != ret) {
        app_err("report device property send queue fail:%d", ret);
    }
}

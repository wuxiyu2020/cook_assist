...
static void timer_service_cb(const char *report_data, const char *property_name, const char *data)
{
    ...
    if (property_name != NULL) {	/* set value to device */
        app_info("timer event callback=%s val=%s", property_name, data);
        ...
        double d_value, const char *s_value, int prop_idx)
{
    if (prop_idx >= NUM_OF_CONTROL_TARGETS) {
        app_err("prop_idx=%d is out of limit=%d", prop_idx, NUM_OF_CONTROL_TARGETS);
    }
    if (report_data != NULL)	/* post property to cloud */
        user_post_property_json(report_data);
    if (property_name != NULL) {	/* set value to device */
        app_info("timer event callback: property_name=%s prop_idx=%d", property_name, prop_idx);
        if ((prop_idx < num_of_tsl_type[0] && strcmp(control_targets_list[0], property_name) == 0)
            ...
            strcpy(msg.seq, SPEC_SEQ);
            send_msg_to_queue(&msg);
            app_info("timer event callback: int_value=%d", i_value);
            /* set int value */
        } else if (prop_idx < num_of_tsl_type[0] + num_of_tsl_type[1]) {
            app_info("timer event callback: double_value=%f", d_value);
            /* set doube value */
        } else {
            if (s_value != NULL)
                app_info("timer event callback: test_value=%s", s_value);
            /* set string value */
        }
...
static void user_deviceinfo_update(void)
{
    ...
    app_info("Device Info Update Message ID: %d", res);
}
    ...
    if (NULL != (ptr = strchr(seqenceId, '@'))) {
        ...
        if (i == MAX_USER) {
            ...
            if (i == MAX_USER) {
                ...
                app_info("replace new user %s index %d", userId, repalce_index);
            }
        }
    }
    if (isDiscard == 1) {
        app_info("Discard msg !!!");
        return -1;
    }
    if (NULL != seqenceId)
...
static int property_setting_handle(const char *request, const int request_len, recv_msg_t * msg)
{
    ...
    if ((root = cJSON_Parse(request)) == NULL) {
        app_err("property set json parse fail");
        return -1;
    }
    ...
    else if ((item = cJSON_GetObjectItem(root, "LightSwitch")) != NULL && cJSON_IsNumber(item)) {
        msg->powerswitch = item->valueint;
        ret = 0;
    }
#endif
    ...
    else if ((item = cJSON_GetObjectItem(root, DEVICETIMER)) != NULL && cJSON_IsArray(item))
    {
        ...
        app_err("property set unsupport item");
        ret = -1;
    }
        ...
        char **response, int *response_len)
{
    ...
    app_info("Service Request Received, Devid: %d, Service ID: %.*s, Payload: %s", devid, serviceid_len, serviceid,
            request);

    root = cJSON_Parse(request);
    if (root == NULL || !cJSON_IsObject(root)) {
        app_err("JSON Parse Error");
        return -1;
    }
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    app_crit("PropSet Devid:%d,payload:\"%s\"", devid, request);
        ...
        int *response_len)
{
    ...
    app_info("ALCS PropGet, Devid:%d, Request:%s", devid, request);
    ...
    for (index = 0; index < cJSON_GetArraySize(request_root); index++) {
        ...
        app_info("Property ID, index: %d, Value: %s", index, item_propertyid->valuestring);
        if (strcmp("powerstate", item_propertyid->valuestring) == 0) {
            ...
            if (NULL != (local_timer_str = timer_service_property_get("[\"LocalTimer\"]"))) {
                app_info("local_timer %s", local_timer_str);
            ...
            if (NULL != (count_down_list_str = timer_service_property_get("[\"CountDownList\"]"))) {
                app_info("CountDownList %s", count_down_list_str);
                cJSON *property = NULL, *value = NULL;
    ...
    *response = cJSON_PrintUnformatted(response_root);
    if (*response == NULL) {
        app_err("cJSON_PrintUnformatted Error");
        cJSON_Delete(response_root);
        return -1;
    }
    cJSON_Delete(response_root);
    *response_len = strlen(*response);
    ...
    app_info("Property Get Response: %s", *response);
...
static int user_report_reply_event_handler(const int devid, const int msgid, const int code, const char *reply,
        const int reply_len)
{
    app_crit("Post Reply ID(%d)Code(%d)", msgid, code);
...
{
    app_info("Trigger Event Reply Received, Devid: %d, Message ID: %d, Code: %d, EventID: %.*s, Message: %.*s", devid,
...
{
    user_example_ctx_t *user_example_ctx = user_example_get_ctx();
    app_info("Device Initialized, Devid: %d", devid);
...
//     if (type == 0) {
//         app_info("New Firmware Version: %s", version);
...
void user_post_property_json(const char *property)
{
    ...
    app_info("Property post Response: %s", property);
    return;
}
...
{
    app_info("callback user_offline_reset_handler called.");
    vendor_device_unbind();
}
#endif
...
void user_post_property_after_connected(void)
{
    ...
    app_info("Post Event Message ID: %d payload %s", res, property_payload);
}
...
int linkkit_main(void *paras)
{
    ...
    if (argc > 1) {
        ...
        if (tmp >= 60) {
            ...
            app_info("set [max_running_seconds] = %d seconds\n", max_running_seconds);
        }
    }
#endif
    ...
    if ((0 == strlen(master_meta_info.product_key)) || (0 == strlen(master_meta_info.device_name))
        ...
        app_err("No device meta info found...\n");
        while (1) {
    ...
    do {
        ...
        if (user_example_ctx->master_devid < 0) {
            app_warn("IOT_Linkkit_Open Failed, retry after 5s...\n");
            HAL_SleepMs(5000);
        }
    ...
    do {
        ...
        if (res < 0) {
            app_warn("IOT_Linkkit_Connect Failed, retry after 5s...\n");
            #ifdef EN_COMBO_NET
    ...
    while (1) {
        ...
        if (max_running_seconds && (time_now_sec - time_begin_sec > max_running_seconds)) {
            app_info("Example Run for Over %d Seconds, Break Loop!\n", max_running_seconds);
            break;
        }

...
static int ota_image_check(uint32_t image_size)
{
    ...
    {
        app_info("RX:   %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
        ...
        app_info("Need: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
        ...
        app_info("user crc check fail\r\n");
        goto exit;
    }

    app_info("OTA image md5 check success");
...
static int ota_init(void *something)
{
    ...
    app_info("ota init off:0x%08x part:%d len:0x%08x\n", param->off_bp, HAL_PARTITION_OTA_TEMP, param->len);
...
static int ota_boot(void *something)
{
    ...
    if (param->res_type == OTA_FINISH)
    {
        ...
        else
        {
            ...
            app_info("ota finished, switch to new firmware ...");
    ...
    HAL_Kv_Set("ota_m_name_1", module_name_value, strlen(module_name_value), 1);
    HAL_Kv_Set("ota_m_version_1",module_version_value, strlen(module_version_value), 1);
    #endif
...
ota_hal_module_t ota_hal_module1 = {
    .init = ota_init,
    .write = ota_write,
    .read = ota_read,
    .boot = ota_boot,

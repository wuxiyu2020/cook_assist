...
void log_print_simple(unsigned int log_level, const char * mode, const char *fmt, ...)
{
    va_list args;
    int ret;
    char level_prt;
    ...
    printf("[%s]<%c>(%s) ", get_current_time(), level_prt, mode);
    va_start(args, fmt);
#ifdef CONFIG_WIRELESS_LOG
    log_wireless_write(level_prt, mode, log_time, fmt, &args);
#endif
    ret = vprintf(fmt, args);
    va_end(args);
    fflush(stdout);

...
static kstat_t task_create(ktask_t *task, const name_t *name, void *arg,
                           ...
                           uint8_t autorun, uint8_t mm_alloc_flag,
...
kstat_t krhino_task_create(ktask_t *task, const name_t *name, void *arg,
                           ...
                           uint8_t autorun)
{
    return task_create(task, name, arg, prio, ticks, stack_buf, stack_size,
                       entry, autorun, K_OBJ_STATIC_ALLOC, 0, 0);
}
...
{
    return task_create(task, name, arg, prio, ticks, stack_buf, stack_size,
                       entry, autorun, K_OBJ_STATIC_ALLOC, cpu_num, 1);
}
...
kstat_t task_dyn_create(ktask_t **task, const name_t *name, void *arg,
                        ...
                        task_entry_t entry, uint8_t cpu_num, uint8_t cpu_binded,
                        uint8_t autorun)
{
    kstat_t      ret;
...
{
    return task_dyn_create(task, name, arg, pri, ticks, stack, entry, 0, 0,
                           autorun);
}
...
{
    return task_dyn_create(task, name, arg, pri, ticks, stack, entry, cpu_num,
                           1, autorun);
}
#endif
#endif
...
kstat_t krhino_task_stack_cur_free(ktask_t *task, size_t *free)
{
    ...
    *free = ((size_t)(task->task_stack_base + task->stack_size) - sp) /
            sizeof(cpu_stack_t);
...
static void task_mutex_free(ktask_t *task)
{
    ...
    while ((mutex = next_mutex) != NULL) {
        ...
        if (!is_klist_empty(blk_list_head)) {
            next_task =
              krhino_list_entry(blk_list_head->next, ktask_t, task_list);
...
kstat_t krhino_task_del(ktask_t *task)
{
    ...
    res_free = (res_free_t *)(task->task_stack_base + task->stack_size -
                              (sizeof(res_free_t) / sizeof(cpu_stack_t)) - 1u);
#endif
    res_free->cnt = 0;
...
void krhino_task_deathbed(void)
{
    ...
    printf("krhino_task_deathbed: prio=%d name=%s task_stack=%ld\r\n", task->prio, task->task_name, task->stack_size);

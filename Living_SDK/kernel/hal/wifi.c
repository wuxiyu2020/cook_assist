...
/**
 * @brief   设置当前WiFi Combo的射频共存模式，仅适用于单天线多模共存芯片
 *
 * @param[in] coex_mode: WiFi&BT combo chip's coexistence mode to be set
 * 
 * @return
 @verbatim
    = 0: succeeded
...
* @see None.
* @note None.
* @note combo application like mesh gateway use this API to set coexistence mode
*/
int hal_wifi_coex_mode_set(hal_wifi_coexmode_t coex_mode)
{
    hal_wifi_module_t *module;

    module = hal_wifi_get_default_module();
    if (module == NULL || module->coex_mode_set == NULL) {
        return -1;
    }
    return module->coex_mode_set(module, coex_mode);
}

/**
 * @brief   设置当前WiFi Combo的射频共存参数，仅适用于单天线多模共存芯片
 *
 * @param[in] coex_mode: WiFi&BT combo chip's coexistence mode to set
 * @param[in] param1: WiFi&BT combo chip's coexistence parameters to set
 * @param[in] param2: WiFi&BT combo chip's coexistence parameters to set
 * 
 * @return
 @verbatim
    = 0: succeeded
...
* @see None.
* @note None.
* @note combo application like mesh gateway use this API to set coexistence parameters
*/
int hal_wifi_coex_param_set(hal_wifi_coexmode_t coex_mode, uint16_t param1, uint16_t param2)
{
    hal_wifi_module_t *module;

    module = hal_wifi_get_default_module();
    if (module == NULL || module->coex_param_set == NULL) {
        return -1;
    }
    return module->coex_param_set(module, coex_mode, param1, param2);
}

...
static int g_uart_rx_dma_chan = -1;
static uart_dma_rx_cb g_uart_rx_dma_cb = NULL;

static void hal_uart_dma_int_handler_rx(void)
{
    uart_dev_t *uart;
    
    if (g_uart_rx_dma_chan >= 0)
    {
        bl_dma_int_clear(g_uart_rx_dma_chan);

        /* Stop */
        DMA_Channel_Disable(g_uart_rx_dma_chan);
        
        uart = (uart_dev_t *)bl_dma_find_ctx_by_channel(g_uart_rx_dma_chan);
        if (g_uart_rx_dma_cb != NULL) {
            g_uart_rx_dma_cb(uart);
        }
    }
}

int32_t hal_uart_dma_recv(uart_dev_t *uart, void *data, uint32_t size)
{
    UART_FifoCfg_Type fifocfg;
    ...
    g_uart_rx_dma_chan = DMA_CH2;
    
    fifocfg.txFifoDmaThreshold = 16;
    fifocfg.rxFifoDmaThreshold = 1;
    if (uart->port == 0) {
    ...
    /* Config DMA */
    DMA_LLI_Ctrl_Type uart_lli_rx = {0};
    struct DMA_Control_Reg uart_rx_dmaCtrlRegVal = {0};
    DMA_LLI_Cfg_Type uart_rx_lliCfg = {0};

    DMA_Channel_Disable(g_uart_rx_dma_chan);

    DMA_IntMask(g_uart_rx_dma_chan, DMA_INT_ALL, MASK);
    DMA_IntMask(g_uart_rx_dma_chan, DMA_INT_TCOMPLETED, UNMASK);
    DMA_IntMask(g_uart_rx_dma_chan, DMA_INT_ERR, UNMASK);
    bl_dma_irq_register(g_uart_rx_dma_chan, &hal_uart_dma_int_handler_rx, NULL, uart);
    bl_irq_enable(DMA_ALL_IRQn);

    uart_rx_dmaCtrlRegVal.TransferSize = size,
    uart_rx_dmaCtrlRegVal.SBSize = DMA_BURST_SIZE_1,
    uart_rx_dmaCtrlRegVal.DBSize = DMA_BURST_SIZE_1,
    uart_rx_dmaCtrlRegVal.SWidth = DMA_TRNS_WIDTH_8BITS,
    uart_rx_dmaCtrlRegVal.DWidth = DMA_TRNS_WIDTH_8BITS,
    uart_rx_dmaCtrlRegVal.SI = DMA_MINC_DISABLE,
    uart_rx_dmaCtrlRegVal.DI = DMA_PINC_ENABLE,
    uart_rx_dmaCtrlRegVal.I = 1,

    uart_lli_rx.srcDmaAddr = (uint32_t)(uart_base[uart->port] + UART_FIFO_RDATA_OFFSET);
    uart_lli_rx.destDmaAddr = (uint32_t)data;
    uart_lli_rx.nextLLI = 0;
    uart_lli_rx.dmaCtrl = uart_rx_dmaCtrlRegVal;

    uart_rx_lliCfg.dir = DMA_TRNS_P2M;
    uart_rx_lliCfg.dstPeriph = DMA_REQ_NONE;
    if (uart->port == 0) {
        uart_rx_lliCfg.srcPeriph = DMA_REQ_UART0_RX;
    } else {
        uart_rx_lliCfg.srcPeriph = DMA_REQ_UART1_RX;
    }

    DMA_LLI_Init(g_uart_rx_dma_chan, &uart_rx_lliCfg);
    DMA_LLI_Update(g_uart_rx_dma_chan, (uint32_t)&uart_lli_rx);
    DMA_Channel_Enable(g_uart_rx_dma_chan);
}

int32_t hal_uart_dma_recv_cb_reg(uart_dev_t *uart, uart_dma_rx_cb cb, void *arg)
{
    if (uart == NULL) {
        return -1;
    }

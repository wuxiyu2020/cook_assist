...
static void report_wifi_tlv_data(void)
{
    struct sm_connect_tlv_desc *ele = NULL;
    /* Get every ele in diagnose tlv data */
    uint8_t *buff = NULL;

    buff          = aos_malloc(8 + 227); //Max 235 due to breeze port limit

    printf("start wifi_mgmr_diagnose_tlv_get_ele:%p\r\n", wifi_mgmr_diagnose_tlv_get_ele());
    while ((ele = wifi_mgmr_diagnose_tlv_get_ele())) {
        os_printf("diagnose tlv id: %d, len: %d, data: %p\r\n", ele->id, ele->len, ele->data);
    ...
    printf("end wifi_mgmr_diagnose_tlv_get_ele\r\n");
    if (NULL != buff) {
        os_printf("[APP] [EVT] free=%p\r\n", buff);
        aos_free(buff);
    }
...
static int wifi_init(hal_wifi_module_t *m) {
  ...
  //add by zhoubw@220709 for debug
  // unsigned char ucMac[] = {0xac, 0xd8, 0x29, 0x73, 0x34, 0x77};
  unsigned char ucMac[6] = {0};
  int mac_len = 6;
  if (0 == aos_kv_get("USER_MAC", ucMac, &mac_len)){
    bl_wifi_sta_mac_addr_set(ucMac);
  }

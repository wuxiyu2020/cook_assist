...
int bl_uart_init(uint8_t id, uint8_t tx_pin, uint8_t rx_pin, uint8_t cts_pin, uint8_t rts_pin, uint32_t baudrate)
{
    ...
    /* enable clk */
    if (0 == uart_clk_init) {
        GLB_Set_UART_CLK(1, HBN_UART_CLK_160M, 7);
        uart_clk_init = 1;
    }
...
void bl_uart_setconfig(uint8_t id, uint32_t baudrate, UART_DataBits_Type databits,
                       UART_StopBits_Type stopbits, UART_Parity_Type parity)
{
    UART_CFG_Type UartCfg =
    {
        20*1000*1000,                                       /* UART clock */
...
{
    bl_uart_setconfig(id, baud, UART_DATABITS_8, UART_STOPBITS_1, UART_PARITY_NONE);
}

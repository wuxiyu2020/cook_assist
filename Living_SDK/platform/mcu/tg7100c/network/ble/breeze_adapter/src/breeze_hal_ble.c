...
static void connected(struct bt_conn *conn, uint8_t err)
{
    if (err) {
        ...
        dump_awss_status(SUB_ERRCODE_BLE_CONNECT_FAIL, "ble connect fail:0x%02x", err);
    } else {
        printf("Connected\n");
...
ais_err_t ble_stack_init(ais_bt_init_t *info)
{
    ...
    if (err)
    {
        return AIS_ERR_GATT_REGISTER_FAIL;
    }
...
ais_err_t ble_advertising_start(ais_adv_init_t *adv)
{
    ...
    // Bind and silent use slow adv
    if ((adv->vdata.data[3] & (1 << 7)) || (adv->vdata.data[3] & (1 << 5))) {

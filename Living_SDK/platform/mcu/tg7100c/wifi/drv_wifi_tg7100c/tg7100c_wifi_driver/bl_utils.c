...
void tg710x_firmwre_mpdu_free(void *swdesc);
    //bl_os_printf("--- cb free@%p\r\n", my_pbuf->swdesc);
    tg710x_firmwre_mpdu_free(my_pbuf->swdesc);
}
...
int tcpip_stack_input(void *swdesc, uint8_t status, void *hwhdr, unsigned int msdu_offset, struct wifi_pkt *pkt)
{
...
#if defined(CFG_CHIP_BL808) || defined(CFG_CHIP_BL606P) || defined(CFG_CHIP_TG7100C)
    ...
    if (sniffer) {
        ...
        bl_rx_pkt_cb((uint8_t*)skb_payload, hw_rxhdr->hwvect.len, (void *)h, hw_rxhdr->hwvect.rssi1);

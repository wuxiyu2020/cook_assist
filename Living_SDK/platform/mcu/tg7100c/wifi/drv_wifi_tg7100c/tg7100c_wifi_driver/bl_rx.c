...
static int bl_rx_apm_sta_add_ind(struct bl_hw *bl_hw, struct bl_cmd *cmd, struct ipc_e2a_msg *msg)
{
    ...
    aos_post_event(TG7100C_EV_WIFI, CODE_WIFI_ON_AP_STA_ADD, ind->sta_idx);
...
static int bl_rx_apm_sta_del_ind(struct bl_hw *bl_hw, struct bl_cmd *cmd, struct ipc_e2a_msg *msg)
{
    ...
    aos_post_event(TG7100C_EV_WIFI, CODE_WIFI_ON_AP_STA_DEL, ind->sta_idx);
...
void bl_rx_pkt_cb(uint8_t *pkt, int len, void *pkt_wrap, int rssi)
{
    if (cb_pkt) {
        cb_pkt(cb_pkt_env, pkt, len, rssi);
    }
    if (cb_pkt_adv) {
        cb_pkt_adv(cb_pkt_env, pkt_wrap);
    }
}

...
static int8_t wifi_rf_table_c13sg[48] = {16, 16, 16, 16, 00, 00, 00, 00,                         //power dbm for 11b 1Mbps/2Mbps/5Mbps/11Mbps 
                                        14, 14, 14, 14, 14, 14, 14, 14,                         //power dbm for 11g 6,9,12,18,24,36,48,54 Mbps 
                                        13, 13, 13, 13, 13, 13, 13, 13,                         //power dbm for 11n MCS0~MCS7 
                                        01, 01, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, 00, //power re-cal for channel 1~14 
                                        36 //capcode 
};

static void wifi_rf_init(int8_t table[48]) 
{ 
    ...
    if (!capcode_slot2 || !capcode_slot1) { 
        int8_t pf[14]; 
        uint8_t capcode; 
        if(bl_efuse_read_pwroft(pf) == 0) memcpy(table+24, pf, 14); 
        if(bl_efuse_read_capcode(&capcode) == 0) table[38] = (int8_t)capcode;
    }
...
static void xm_rf_init()
{
    char m_factory[32] = {0};
    int temp_len = 32;
    int ret = aos_kv_get("user_fact", m_factory, &temp_len);
    if ((ret == 0) && (strlen(m_factory) > 0))
    {
        printf("模组厂商代码:  (%s)\r\n", m_factory);
        if (0 == strncmp(m_factory, "XM-X-C13SG", sizeof("XM-X-C13SG")))
        {
            printf("开始设置RF参数......\r\n");
            wifi_rf_init(wifi_rf_table_c13sg);
        }
    }
    else
    {

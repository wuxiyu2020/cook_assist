                                ...
                                hal_wifi_ip_stat_t *pnet, void *arg)
{
    ...
    wifi_get_ip(ip);
    dump_device_state(STATE_CODE_WIFI_GOT_IP, "got ip:%s", ip);
#endif
}
                                  ...
                                  void *arg)
{
    ...
    switch (stat) {
        ...
        {
            static uint8_t pw_err_count = 0;
            static uint8_t ssid_not_found_count = 0;
            static uint8_t connect_ap_other_fail_count = 0;
            ...
            if (reason_code == 6 || reason_code == 8 || reason_code == 9 || reason_code == 10 || reason_code == 11)
            {
                pw_err_count++;
                if (pw_err_count >= ERROR_COUNT_MAX)
                {
                    pw_err_count = 0;
                    dump_awss_status(SUB_ERRCODE_WIFI_AP_CONN_PASSWD_ERR, "connect ap passwd err rc:%d", reason_code);
                }
            }
            else if (reason_code == 12)
            {
                ssid_not_found_count++;
                if (ssid_not_found_count >= ERROR_COUNT_MAX)
                {
                    ssid_not_found_count = 0;
                    dump_awss_status(SUB_ERRCODE_WIFI_AP_DISCOVER_FAIL, "connect ap found fail rc:%d", reason_code);
                }
            }
            else
            {
                connect_ap_other_fail_count++;
                if (connect_ap_other_fail_count >= ERROR_COUNT_MAX)
                {
                    connect_ap_other_fail_count = 0;
                    dump_awss_status(SUB_ERRCODE_WIFI_AP_CONN_OTHERS_FAIL, "connect ap fail rc:%d", reason_code);
                }
            }
        }
        break;
...
#endif
#ifdef CONFIG_WIRELESS_LOG
    dump_device_state(STATE_CODE_WIFI_CONNECT_AP, "connect AP ssid:%s pwd:%s", ap_config->ssid, type.wifi_key);
#endif
...
static int set_wifi_ssid(void)
{
    ...
    ret = aos_kv_set(NETMGR_WIFI_SS_KEY, (unsigned char *)&g_netmgr_cxt.saved_conf,
                     sizeof(netmgr_ap_config_t), 1);
    if (0 == ret)
    {   /* Remove unsecured config only when secured config save success */
...
#ifdef CONFIG_WIRELESS_LOG
    dump_device_state(STATE_CODE_WIFI_GET_AP_INFO, "get ap info");
#endif
...
static void read_persistent_conf(void)
{
    ...
    if (ret < 0) {
        ...
        return;
#else
        LOGI(TAG, "try secured ap config");
        ret = aos_kv_get(NETMGR_WIFI_SS_KEY, &g_netmgr_cxt.saved_conf, &len);
        if (ret < 0) {
            LOGE(TAG, "no valid AP config");
            return;
        }
    } else {
        LOGI(TAG, "unsecured ap config found!");

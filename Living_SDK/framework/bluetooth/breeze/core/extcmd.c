...
enum
{
    BLE_BREEZE_GET_CACHED_LOG = 0x01,
    BLE_BREEZE_START_REAL_LOG = 0x02,
    BLE_BREEZE_GET_CACHED_AND_REAL_LOG = 0x03,
    BLE_BREEZE_STOP_REAL_LOG = 0x04,
    BLE_BREEZE_SET_REAL_LOG_LEVEL_ERR = 0x05,
    BLE_BREEZE_SET_REAL_LOG_LEVEL_WARN = 0x06,
    BLE_BREEZE_SET_REAL_LOG_LEVEL_INFO = 0x07,
};
...
static ret_code_t ext_cmd06_rsp(uint8_t *p_buff, uint8_t *p_blen, const uint8_t *p_data, uint8_t dlen)
{
    ...
    dev_awss_state_set(AWSS_PATTERN_BLE_CONFIG, AWSS_STATE_START);
#endif
    ...
    *p_blen = sizeof(rsp);
    ...
    {
        dump_awss_status(SUB_ERRCODE_AWSS_SSID_PWD_PARSE_FAIL, "ble awss parse ssid and pwd fail:%d", err_code);
    }
...
static ret_code_t ext_cmd07_rsp(uint8_t *p_buff, uint8_t *p_blen, const uint8_t *p_data, uint8_t dlen)
{
    ...
    switch (value) {
    case BLE_BREEZE_GET_CACHED_LOG: {
        log_bucket_set_log_type(REPROT_LOG_TYPE_CACHED);
    } break;
    case BLE_BREEZE_START_REAL_LOG: {
        log_bucket_set_log_type(REPROT_LOG_TYPE_REAL);
    } break;
    case BLE_BREEZE_GET_CACHED_AND_REAL_LOG: {
        log_bucket_set_log_type(REPROT_LOG_TYPE_CACHED_AND_REAL);
    } break;
    case BLE_BREEZE_STOP_REAL_LOG: {
        log_bucket_send_real_stop();
    } break;
    case BLE_BREEZE_SET_REAL_LOG_LEVEL_ERR: {
        log_bucket_set_real_log_level(REAL_LOG_LEVEL_ERROR);
    } break;
    case BLE_BREEZE_SET_REAL_LOG_LEVEL_WARN: {
        log_bucket_set_real_log_level(REAL_LOG_LEVEL_WARNNING);
    } break;
    case BLE_BREEZE_SET_REAL_LOG_LEVEL_INFO: {
        log_bucket_set_real_log_level(REAL_LOG_LEVEL_INFO);
    } break;
    default:
        BREEZE_WARN("wireless log unknown type");
        break;
    }

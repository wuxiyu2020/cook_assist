...
void do_decrypt(uint8_t *data, uint16_t *len)
{
    ...
    BREEZE_VERBOSE("解密后数据如下:");
    hex_byte_dump_verbose(decrypt_data, *len, 24);
}
...
static ret_code_t send_fragment(void)
{
    ...
    do {
        ...
        if(ret == BZ_EINVALIDSTATE)/*ble gatt disconnected*/
        {
            g_transport.tx.len = 0;
            g_transport.tx.bytes_sent = 0;
            g_transport.tx.msg_id = 0;
...
static void trans_rx_dispatcher(void)
{
    ...
    if (g_transport.rx.bytes_received > 1)
    {
        uint16_t buff_str_len = g_transport.rx.bytes_received * 2 + 1;
        char *buff_str = HAL_Malloc(buff_str_len);

        if (buff_str)
        {
            memset(buff_str, 0, buff_str_len);
            hex2string(g_transport.rx.buff, g_transport.rx.bytes_received, buff_str);
            BREEZE_INFO("breeze rx cmd(0x%02x)len(%d)pdu:%s", g_transport.rx.cmd, g_transport.rx.bytes_received, buff_str);
            HAL_Free(buff_str);
        }
    }
    else
    {
        BREEZE_INFO("breeze rx cmd(0x%02x)", g_transport.rx.cmd);
    }
...
extern aos_mutex_t g_ble_trans_tx_mutex;
static uint64_t transport_tx_time_last = 0;  //time_mem = aos_now_ms();
ret_code_t transport_tx(uint8_t tx_type, uint8_t cmd,
                        uint8_t const *const p_data, uint16_t length)
{
    aos_mutex_lock(&g_ble_trans_tx_mutex, AOS_WAIT_FOREVER);
    uint64_t time_diff = aos_now_ms() - transport_tx_time_last;
    if (time_diff < 500)
    {
        BREEZE_VERBOSE("transport_tx: ble need sleep %d ms", 500 - time_diff);
        aos_msleep(500 - time_diff);
    }
    ...
    if(cmd != BZ_CMD_ERR){
        // Parameters check
        if (p_data == NULL) {
            aos_mutex_unlock(&g_ble_trans_tx_mutex);
            return BZ_ENULL;
        }
    ...
    if (g_transport.p_key != NULL &&
        (cmd == BZ_CMD_REPORT || cmd == BZ_CMD_RESPONSE || cmd == BZ_CMD_INDICATE
         ...
         || cmd == BZ_CMD_AUTH_RPT_CIPHER || cmd == BZ_CMD_WIRELESS_LOG)) {
    ...
    if (tx_bytes_left() != 0 ||
        ...
        BREEZE_ERR("It deserves our attention!!! (%d %d %d)", tx_bytes_left(), g_transport.tx.pkt_req, g_transport.tx.pkt_cfm);
        aos_mutex_unlock(&g_ble_trans_tx_mutex);
        return BZ_EBUSY;
    }
    ...
    if (cmd == BZ_CMD_RESPONSE || cmd == BZ_CMD_MANU_RESP || cmd == BZ_CMD_EXT_UP) {
        ...
        BREEZE_INFO("use default tx.msg_id:%d", g_transport.tx.msg_id);
    }
    ...
    if(p_data != NULL && length != 0){
        ...
        uint16_t buff_str_len = length * 2 + 1;
        char *buff_str = HAL_Malloc(buff_str_len);

        if (buff_str)
        {
            memset(buff_str, 0, buff_str_len);
            hex2string(p_data, length, buff_str);
            BREEZE_INFO("breeze tx type:%d cmd(0x%02x)pdu:%s", tx_type, cmd, buff_str);
            HAL_Free(buff_str);
        }
    }
    ...
    send_fragment();
    transport_tx_time_last = aos_now_ms();
    aos_mutex_unlock(&g_ble_trans_tx_mutex);
    return BZ_SUCCESS;
}

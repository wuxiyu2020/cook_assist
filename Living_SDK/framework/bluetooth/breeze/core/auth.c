...
void auth_rx_command(uint8_t cmd, uint8_t *p_data, uint16_t length)
{
    ...
    BREEZE_INFO("auth cmd(0x%02x)state:%d", cmd, g_auth.state);
    switch (g_auth.state) {
        case BZ_AUTH_STATE_SVC_ENABLED:
            ...
            if (cmd == BZ_CMD_AUTH_SET_RAND) {
                ...
                hex_byte_dump_verbose(g_auth.ikm, g_auth.ikm_len, 24);
                update_aes_key(true);
                update_trans_key();
    ...
    if (g_auth.state == IOTB_AUTH_STATE_DONE) {
        ...
        os_timer_stop(&g_auth.timer);
        uint8_t buf[4] = {0x03, 0x02, 0xE2, 0xC4};      /* send 50402 fatal error */
        transport_tx(TX_INDICATION, BZ_CMD_REPORT, buf, sizeof(buf));
    }
}

/**
 * @file genie_ais.c
 * @brief genie ais for ota
 *
 * @copyright Copyright (C) 2015-2020 Alibaba Group Holding Limited
 *
 */

#define BREEZE_DEBUG_ENABLED IS_ENABLED(CONFIG_BT_MESH_DEBUG_OTA)
#include "genie_ais.h"
#include "genie_ota.h"
...
extern void do_decrypt(uint8_t *data, uint16_t *len);
...
//     {
//         BREEZE_INFO("ais decrypt:%s fail", bt_hex(payload, len));
//         return -1;
//     }
...
static void _ais_indicate_rsp(struct bt_conn *conn, const struct bt_gatt_attr *attr, u8_t err)
{
    ...
    // BREEZE_INFO("ais rsp err(%d) p_ind(%p)", err, p_indicate);
    // if (p_indicate)
    // {
    //     aos_free(p_indicate);
    //     p_indicate = NULL;
...
static void _ais_server_indicate(uint8_t msg_id, uint8_t cmd, uint8_t *p_msg, uint16_t len)
{
    ...
    BREEZE_DEBUG("len %d: %s", len + 4, bt_hex(&msg, len + 4));
    ...
    if (genie_ais_ctx.p_conn)
    {
        ...
        BREEZE_INFO("ais ind msgid:%d cmd:%02x p_ind:%p len:%d data:%s", msg_id, cmd, p_indicate, len, bt_hex(&msg, len + 4));
#else
        BREEZE_INFO("ais ind msgid:%d cmd:%02x p_ind:%p len:%d", msg_id, cmd, p_indicate, len);
#endif

        ret = bt_gatt_indicate(genie_ais_ctx.p_conn, p_indicate);
        if (ret < 0)
        {
            BREEZE_ERR("ais bt_gatt_indicate err:%d", ret);
            aos_free(p_indicate);
        }
    }
    else
    {
        BREEZE_WARN("ais not connect");
        aos_free(p_indicate);
    }
}
...
void genie_ais_notify(uint8_t msg_id, uint8_t cmd, uint8_t *p_msg, uint16_t len, uint8_t enc)
{
    ...
    // BREEZE_INFO("len %d: %s", len + 4, bt_hex(&msg, len + 4));

    if (genie_ais_ctx.p_conn)
    {
#ifdef DEBUG_AIS_DETAIL_DATA
        BREEZE_INFO("ais notify msgid:%d cmd:%02x len:%d data:%s", msg_id, cmd, len, bt_hex(&msg, len + 4));
#else
        BREEZE_INFO("ais notify msgid:%d cmd:%02x len:%d", msg_id, cmd, len);
#endif

        // bt_gatt_notify(genie_ais_ctx.p_conn, &_ais_srv_attrs[11], &msg, len + 4);
        // transport_tx(TX_NOTIFICATION, BZ_CMD_OTA_VER_RSP, (uint8_t*)&msg, len + 4);
        ble_ais_send_notification((uint8_t*)&msg, len + 4);
    }
}
...
static void handle_ais_disconnect(void)
{
    if (genie_ais_ctx.p_conn)
    {
        BREEZE_DEBUG("ais handle disconn:%p", genie_ais_ctx.p_conn);
...
{
    BREEZE_DEBUG("ais disconn 0x%x, state %d", reason, genie_ais_ctx.state);
...
static void genie_ais_state_update(void)
{
    ...
    {
        BREEZE_DEBUG("ais update state:%d", genie_ais_ctx.state);
    }
    ...
    BREEZE_INFO("genie_ais_state_update state:%d", genie_ais_ctx.state);
...
{
    BREEZE_DEBUG(" %d", genie_ais_ctx.state);
    switch (genie_ais_ctx.state)
    {
    case AIS_STATE_CONNECT:
    case AIS_STATE_IDLE:
    ...
    case AIS_STATE_REBOOT:
    {
        BREEZE_INFO("ais ota update done,do reboot ...");
        genie_sal_ota_reboot();
    }
    break;
    default:
...
{
    BREEZE_DEBUG("ais connect state:%d conn:%p", genie_ais_ctx.state, p_conn);
    if (genie_ais_ctx.state != AIS_STATE_REBOOT)
    {
        BREEZE_DEBUG("status %d", genie_ais_ctx.state);
...
{
    BREEZE_INFO("ais timeout disconnect");
    handle_ais_disconnect();
}
...
void ais_server_msg_handle(struct bt_conn *p_conn, ais_pdu_t *p_msg, uint16_t len)
{
    bool timer_refresh = false;

    if (p_msg->header.cmd != AIS_OTA_DATA && !_ais_msg_check_header((ais_header_t *)p_msg))
    {
        BREEZE_INFO("ais invalid msg, ignore");
    }

    if (p_msg->header.cmd != AIS_OTA_DATA)
    {
#ifdef DEBUG_AIS_DETAIL_DATA
        BREEZE_INFO("ais cmd:0x%02x state:%d len:%d data:%s", p_msg->header.cmd, genie_ais_ctx.state, len, bt_hex(p_msg, len));
#else
        BREEZE_INFO("ais cmd:0x%02x state:%d len:%d", p_msg->header.cmd, genie_ais_ctx.state, len);
#endif
    }
    ...
    case AIS_OTA_VER_REQ:
    {
        if (len == 20) //This is encrypted data
        {
            ...
            {
                len = 16; 
                do_decrypt(p_msg->payload, &len);
                BREEZE_INFO("do_decrypt len:%d data:%s", len, bt_hex(p_msg->payload, len));
    ...
    default:
    {
        ...
        BT_WARN("ais unsupport cmd %x", p_msg->header.cmd);
    }
        return;
    }
    ...
    else
    {
        if (genie_ais_ctx.state != AIS_STATE_OTA)
        {
            BREEZE_INFO("ais err state:%d cmd:0x%02x len:%d", genie_ais_ctx.state, p_msg->header.cmd, len);
                                ...
                                void *buf, u16_t len, u16_t offset)
{
    ...
    BREEZE_DEBUG("len %d: %s", len, bt_hex(buf, len));
...
{
    //BREEZE_DEBUG("len %d: %s", len, bt_hex(p_buf, len));

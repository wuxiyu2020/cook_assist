...
static uint8_t g_ble_state = 0;
static get_wirelesslog_subcode g_subcode_cb = NULL;

int32_t aiot_set_wirelesslog_code_cb(get_wirelesslog_subcode subcode_cb)
{
    if(subcode_cb == NULL)
    {
        return -1;
    }
    g_subcode_cb = subcode_cb;
    return 0;
}
...
uint32_t breeze_post(uint8_t *buffer, uint32_t length)
{
    ...
    hex_byte_dump_debug(buffer, length, 24);
    return transport_tx(TX_INDICATION, BZ_CMD_REPORT, buffer, length);
}

uint32_t breeze_logpost(uint8_t *buffer, uint32_t length)
{
    BREEZE_DEBUG("breeze_post");
    return transport_tx(TX_INDICATION, BZ_CMD_WIRELESS_LOG, buffer, length);
}
...
uint32_t breeze_logpost_fast(uint8_t *buffer, uint32_t length)
{
    BREEZE_DEBUG("breeze_post_fast");
    return transport_tx(TX_NOTIFICATION, BZ_CMD_WIRELESS_LOG, buffer, length);
}
...
int breeze_start_advertising(uint8_t bind_state, uint8_t silent_flag)
{
    ...
    ais_adv_init_t adv_data = {
        .flag = AIS_AD_GENERAL | AIS_AD_NO_BREDR,
        .name = { .ntype = AIS_ADV_NAME_FULL, .name = "HXR_JCZ" },
    };
    ...
    uint16_t buff_str_len = adv_data.vdata.len * 2 + 1;
    char    *buff_str     = HAL_Malloc(buff_str_len);

    if (buff_str) {
        memset(buff_str, 0, buff_str_len);
        hex2string(adv_data.vdata.data, adv_data.vdata.len, buff_str);
        BREEZE_INFO("breeze adv send bind:%d silent:%d data:%s", bind_state, silent_flag, buff_str);
        HAL_Free(buff_str);
    }
...
int breeze_stop_advertising(void)
{
    ...
    BREEZE_INFO("breeze adv stop ok");
...
int32_t aiot_ais_report_awss_status(ais_awss_status_t status, uint16_t subcode)
{
    ...
    if(g_subcode_cb != NULL)
    {
        last_errsubcode = g_subcode_cb();
    }

    if (g_ble_state == 0) { /*ble disconnect*/
        return -1;
    }

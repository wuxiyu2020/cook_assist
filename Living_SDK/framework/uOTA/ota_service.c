...
#define CHECK_IMG_BUF_SIZE   512
        ...
        // modify by zhoubw @220919
        // ota_reboot();       
        extern void mars_dev_OTANeedReboot(void);
        mars_dev_OTANeedReboot();
        return;
    }
#endif
    ...
    #endif
#if defined (BOARD_ESP8266)
    ctx->upg_status = OTA_REBOOT; /* reboot wifi anyway, when use esp8266 */
#else
    if(!ctx->upg_module_flag) { /* do not reboot wifi when processing mcu firmware upgrade */
...
ERR:
    ...
    #ifdef TG7100CEVB
        #ifdef TG7100C_POWERSAVE_ENABLE
            extern int get_cloud_connect_state(void);
            if(get_cloud_connect_state() == 1)

...
int awss_dev_ap_start(void)
{
    ...
    if (g_awss_dev_ap_mutex || awss_dev_ap_ongoing) {
        dump_awss_status(STATE_CODE_WIFI_DEV_AP_ALREADY_RUN, "dev ap already running");
        return STATE_CODE_WIFI_DEV_AP_ALREADY_RUN;
    }
    ...
    if (g_awss_dev_ap_mutex == NULL) {
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_START_FAIL, "mutex create is null");
    ...
    if (STATE_SUCCESS != ret) {
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_START_FAIL, "dev ap setup fail");
        goto AWSS_DEV_AP_FAIL;
    }
#ifdef AWSS_BATCH_DEVAP_ENABLE
    {
...
#define AWSS_DEV_AP_SWITCHA_RSP_LEN (512)
    ...
    if (0 == awss_dev_ap_ongoing) {
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_RECV_IN_WRONG_STATE, "not in awss mode");
        return -1;
    }
    ...
    if (msg == NULL) {
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_PARSE_PKT_FAIL, "switchap resp alloc fail");
        goto DEV_AP_SWITCHAP_END;
    }
    ...
    if ((switchap_JSON == NULL) || (!cJSON_IsObject(switchap_JSON))){
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_RECV_PKT_INVALID, "switchap req json parse fail");
        goto DEV_AP_SWITCHAP_END;
    }
    ...
    if (str == NULL) {
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_RECV_PKT_INVALID, "switchap req param fail");
        goto DEV_AP_SWITCHAP_END;
    }
    ...
    do {
        ...
        if (!ssid_found) {
            ...
            if (str && (strlen(str) < PLATFORM_MAX_SSID_LEN * 2 - 1)) {
                ...
                dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_RECV_PKT_INVALID, "switchap req ssid err");
        ...
        if (str && strlen(str) ==  RANDOM_MAX_LEN * 2) {
            ...
            dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_RECV_PKT_INVALID, "switchap req random len err");
        ...
        if (strlen(str) < (PLATFORM_MAX_PASSWD_LEN * 2) - 1) {
            ...
            if (isRandomKey) {
                if (softap_decrypt_password(encoded, (const uint8_t*)p_random_str, passwd) < 0) {
                    success = 0;
                    dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_PASSWD_DECODE_FAILED, "randomkey passwd decode fail");
                ...
                if (aes_decrypt_string(encoded, passwd, strlen(str), 0, os_get_encrypt_type(), 1, random) < 0) {
                    /* 64bytes=2x32bytes */
                    success = 0;
                    dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_PASSWD_DECODE_FAILED, "non-random passwd decode");
                    awss_event_post(IOTX_AWSS_PASSWD_ERR);
                }
            }
        } else {
            dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_PASSWD_DECODE_FAILED, "passwd len err");
        ...
        if (success && zconfig_is_utf8(passwd, strlen(passwd)) == 0) {
            dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_PASSWD_DECODE_FAILED, "passwd content err");
    ...
    if (0 != result) {
        dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_SEND_PKT_FAIL, "send switchap resp fail:%d", result);
    }
...
int wifimgr_process_dev_errcode_request(void *ctx, void *resource, void *remote, void *request)
{
    ...
    char *version = NULL;
    int ret = 0;
    uint16_t err_code = DEV_ERRCODE_SYSTERM_ERR;
...
static void do_connect_ap(void)
{
    ...
    if (awss_dev_ap_switchap_resp_suc || ++ap_info_ptr->cnt == TIMEOUT_CNT) {
        ...
        if (0 != os_awss_close_ap()) {
            dump_awss_status(SUB_ERRCODE_WIFI_DEV_AP_CLOSE_FAIL, "stop dev ap fail");
        }
#ifdef AWSS_BATCH_DEVAP_ENABLE
        {
            uint8_t alibaba_oui[3] = WLAN_OUI_ALIBABA_ARRAY;
        ...
        if (ret == 0) {
            ...
            dump_awss_status(STATE_CODE_WIFI_CONNECT_AP_SUCCESS, "connect ssid:%s success", ap_info_ptr->ssid);
        } else {
...
{
    if (ap_info_ptr == NULL) {
        dump_awss_status(SUB_ERRCODE_USER_INPUT_NULL_POINTER, "conn ap ap_info_ptr null");

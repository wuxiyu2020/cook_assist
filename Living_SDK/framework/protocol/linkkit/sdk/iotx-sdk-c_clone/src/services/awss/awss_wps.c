...
#define EXTRA_LEN    (3)   // ssid_len(1B) + checksum(2B)
    ...
    if (crc != cal_crc) {
        ...
        dump_awss_status(STATE_CODE_WIFI_P2P_DEBUG, "rx illegal p2p (0x%x != 0x%x)", crc, cal_crc);
...
#define MAC_LOCAL_ADMINISTERED_BIT    (0x02)
    ...
    if (zc_android_src[0] & MAC_LOCAL_ADMINISTERED_BIT) {
        ...
        // dump_awss_status(STATE_CODE_WIFI_P2P_DEBUG, "android src: %02x%02x%02x", zc_android_src[0], src[1], src[2]); 
    } else {
        dump_awss_status(STATE_CODE_WIFI_P2P_DEBUG, "local administered bit not set: %02x%02x%02x",
                         src[0], src[1], src[2]);
    }
    ...
    switch (encrypt) {
        case P2P_ENCODE_TYPE_ENCRYPT: {
            ...
            if (zconfig_is_utf8((const char *)tmp_passwd, passwd_len) == 0) {
                ...
                dump_awss_status(STATE_CODE_WIFI_P2P_DEBUG, "p2p decrypt passwd content err");
        ...
        default: {
            void *mutex = zc_mutex;
            dump_awss_status(STATE_CODE_WIFI_P2P_DEBUG, "p2p encypt:%d not support", encrypt);
            memset(zconfig_data, 0, sizeof(*zconfig_data));
            zc_mutex = mutex;
            return GOT_NOTHING;
        }
    ...
    if (ssid_truncated) {
        ...
        if (pre_ssid_len && pre_ssid_len < cur_ssid_len) {
            /* should not happen */
            dump_awss_status(STATE_CODE_WIFI_P2P_DEBUG, "pre:%s < cur:%s", zc_pre_ssid, tmp_ssid);

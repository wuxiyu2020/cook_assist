...
int __awss_start(void)
{
    ...
    if (!ret) {
        awss_info("awss no ssid get, timeout or force stoped");
    }
    ...
    do {
        ...
        if (!ret) {
            ...
            dump_awss_status(STATE_CODE_WIFI_CONNECT_AP_SUCCESS, "awss connect ssid:%s success", ssid);
        } else {

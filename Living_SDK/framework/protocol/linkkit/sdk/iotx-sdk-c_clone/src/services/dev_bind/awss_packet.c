...
static void * awss_get_dev_info(void *dev_info, int len)
{
    ...
    if (dev_info == NULL || len <= 0) {
        dump_awss_status(SUB_ERRCODE_USER_INPUT_NULL_POINTER, "get dev info input null");
        return NULL;
    }
    ...
    if (ret <= 0) {
        dump_awss_status(SUB_ERRCODE_OTA_FIRMWARE_VER_READ_FAIL, "FW Version get failed");
    }
...
void *awss_build_dev_info(int type, void *dev_info, int info_len)
{
    ...
    if (dev_info == NULL || info_len <= 0) {
        dump_awss_status(SUB_ERRCODE_USER_INPUT_NULL_POINTER, "build dev info input null!");
        return NULL;
    }

    buf = os_zalloc(DEV_INFO_LEN_MAX);
    if (buf == NULL) {
        dump_awss_status(SUB_ERRCODE_SYS_DEPEND_MALLOC, "build dev info alloc fail");
        return NULL;
    }
...
int awss_build_packet(int type, void *id, void *ver, void *method, void *data, int code, void *packet, int *packet_len)
{
    ...
    return SUB_ERRCODE_USER_INPUT_OUT_RANGE;
}

    ...
    uint8_t *out_token_type)
{
    ...
    if ( (ie_len <= ie_idx) || ((ie[ie_idx] & 0x0F) != WLAN_VENDOR_DEVTYPE_ALINK_CLOUD) ) {
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi type=%d unmatch", ie[ie_idx]);
        return SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR;
    }
    ...
    if ( (ie_len <= ie_idx + ie[ie_idx]) || !g_dev_sign || memcmp(g_dev_sign, p_dev_name_sign + 1, p_dev_name_sign[0])) {
        ...
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi dn unmatch");
        //awss_debug("expect:");
    ...
    if ( (ie_len <= ie_idx) || (ie[ie_idx] != REGISTRAR_FRAME_TYPE) ) {
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi frametype=%d unmatch", ie[ie_idx]);
        return SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR;
    }
    ...
    if ( (ie_len <= ie_idx + ie[ie_idx]) || (ie[ie_idx] >= OS_MAX_SSID_LEN) ) {
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi ssidlen=%d out of range", ie[ie_idx]);
        return SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR;
    }
    ...
    if ( (ie_len <= ie_idx + ie[ie_idx]) || p_passwd[0] >= OS_MAX_PASSWD_LEN) {
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi passwdlen=%d out of range", p_passwd[0]);
        return SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR;
    }
    ...
    if ( ie_len < ie_idx + ETH_ALEN) {
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi bssid out of range");
        return SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR;
    }
    ...
    if (zconfig_is_utf8((const char *)tmp_passwd, p_passwd[0]) != 1) {
        ...
        dump_awss_status(SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR, "enr_hdl_regi(passwd invalid");
        return SUB_ERRCODE_WIFI_ZCONFIG_REGISTAR_PARAMS_ERROR;
    }

    ...
    if (zc_ssid[0] != '\0' && (flag & SSID_EXIST_MASK)
        ...
        if (!(flag & SSID_ENCODE_MASK)) {/* ASCLL ssid */
            if ((ssid_len | 0x200) != pkg_len(3)) {
                dump_awss_status(STATE_CODE_WIFI_PROCESS_FRAME, "ssid len not match! ssid:%s != %d", 
...
int zconfig_get_ssid_passwd(uint8_t tods)
{
    ...
    if (passwd_encrypt == PASSWD_ENCRYPT_CIPHER || passwd_encrypt == PASSWD_ENCRYPT_OPEN) {
        dump_awss_status(SUB_ERRCODE_WIFI_BCAST_UNSUPPORT, "!aes128-cfb is not support: flag 0x%x", flag);
        ret = SUB_ERRCODE_WIFI_BCAST_UNSUPPORT;
        goto exit;
    } else {
    ...
    if (crc != cal_crc) {
        dump_awss_status(SUB_ERRCODE_WIFI_CRC_ERROR, "crc error: recv 0x%x != 0x%x", crc, cal_crc);
    ...
    if (passwd_encrypt > PASSWD_ENCRYPT_CIPHER) {
        ...
        if (zconfig_is_utf8((const char *)zc_passwd, passwd_len) == 0) {
            void *mutex = zc_mutex;
            dump_awss_status(SUB_ERRCODE_WIFI_PASSWD_DECODE_FAILED, "passwd err");
            if (zconfig_data != NULL) {
                memset(zconfig_data, 0, sizeof(*zconfig_data));
            }
            zc_mutex = mutex;
            ...
            ret = SUB_ERRCODE_WIFI_PASSWD_DECODE_FAILED;
            goto exit;
        }
    } else {
        void *mutex = zc_mutex;
        ...
        dump_awss_status(SUB_ERRCODE_WIFI_BCAST_UNSUPPORT, "encrypt:%d not support", passwd_encrypt);
        if (zconfig_data != NULL) {
            memset(zconfig_data, 0, sizeof(*zconfig_data));
        }
        zc_mutex = mutex;
...
found_match:
    ...
    do {
        ...
        if (ap_info && ap_info->encry[tods] > ZC_ENC_TYPE_MAX)
            dump_awss_status(STATE_CODE_WIFI_PROCESS_FRAME, "invalid apinfo ssid:%s", ap_info->ssid);
                                       ...
                                       signed char rssi)
{
    ...
    if (encry == ZC_ENC_TYPE_INVALID) {
        dump_awss_status(STATE_CODE_WIFI_PROCESS_FRAME, "invalid encry type!");
	}

...
{
    if (awss_stopped == 0) {
        dump_awss_status(SUB_ERRCODE_WIFI_IN_PROGRESS, "awss already running");
        return SUB_ERRCODE_WIFI_IN_PROGRESS;
    }
    ...
    if (awss_stopped) {
        dump_awss_status(STATE_CODE_WIFI_FORCE_STOPPED, "awss stopped in %s", __func__);
        return STATE_CODE_WIFI_FORCE_STOPPED;
    }

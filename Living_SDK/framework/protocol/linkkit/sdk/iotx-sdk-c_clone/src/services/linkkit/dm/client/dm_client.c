                            ...
                            int dev_type)
{
    ...
    for (; index < number; index++) {
        ...
        if (g_dm_client_uri_map[index].uri_name && !strncmp((char *)g_dm_client_uri_map[index].uri_name, DM_URI_THING_EVENT_NOTIFY, strlen(DM_URI_THING_EVENT_NOTIFY)))
        {
            res = dm_utils_service_name((char *)g_dm_client_uri_map[index].uri_prefix, (char *)g_dm_client_uri_map[index].uri_name,
                                        product_key, device_name, &uri); /* plus sign wildcards used */
            if (res < SUCCESS_RETURN)
            {
                continue;
            }

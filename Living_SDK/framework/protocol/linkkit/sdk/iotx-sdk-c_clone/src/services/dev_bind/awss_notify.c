...
static int awss_notify_response(int type, int result, void *message)
{
    ...
    if (message == NULL) {
        return SUB_ERRCODE_WIFI_COAP_RSP_INVALID;
    }
...
int awss_notify_dev_info(int type, int count)
{
    ...
    do {
        ...
        if (type == AWSS_NOTIFY_SUCCESS) {
            dump_awss_status(STATE_CODE_WIFI_SENT_CONNECTAP_NOTIFY, "connect ap notify");
...
static int awss_process_get_devinfo()
{
    ...
    do {
        ...
        if (msg == NULL) {
            ret = SUB_ERRCODE_BIND_COAP_MSG_INVALID;
            goto GET_DEV_INFO_ERR;
        }
        ...
        utils_hex_to_str(g_aes_random, RANDOM_MAX_LEN, rand_str, sizeof(rand_str));
        dump_dev_bind_status(SUB_ERRCODE_BIND_SENT_TOKEN_RESP, dev_info);
        ...
        if ((ret = awss_cmp_coap_send_resp(buf, strlen(buf), ctx->remote, topic, ctx->request, NULL, NULL, 0)) < 0) {
            dump_dev_bind_status(SUB_ERRCODE_BIND_APP_GET_TOKEN_RESP_FAIL, topic);
...
GET_DEV_INFO_ERR:
    dump_dev_bind_status(SUB_ERRCODE_BIND_APP_GET_TOKEN_RESP_FAIL, "get token response fail");
    awss_release_coap_ctx(coap_session_ctx);
    coap_session_ctx = NULL;
    awss_stop_timer(get_devinfo_timer);
    get_devinfo_timer = NULL;
                                  ...
                                  void *request, char is_mcast)
{
     ...
     */
    dump_dev_bind_status(SUB_ERRCODE_BIND_RECV_TOKEN_QUERY, "recv token query");
#ifdef DEVICE_MODEL_ENABLED
    if (awss_check_reset(NULL)) {
        dump_dev_bind_status(SUB_ERRCODE_BIND_RST_IN_PROGRESS, "need do reset");
        return SUB_ERRCODE_BIND_RST_IN_PROGRESS;
    }
#endif
    /*
     ...
     */
    if (coap_session_ctx != NULL) {
        dump_dev_bind_status(SUB_ERRCODE_BIND_COAP_INIT_FAIL, "recv get dev info fail");
        return SUB_ERRCODE_BIND_COAP_INIT_FAIL;
    }
    /*
    ...
    if (coap_session_ctx == NULL) {
        dump_dev_bind_status(SUB_ERRCODE_BIND_COAP_INIT_FAIL, "cpy req ctx fail");
        return SUB_ERRCODE_BIND_COAP_INIT_FAIL;
    }
...
static int __awss_suc_notify()
{
    ...
    if (success_notify_mutex == NULL) {
        ...
        if (success_notify_mutex == NULL) {
            return SUB_ERRCODE_SYS_DEPEND_MUTEX_CREATE;
        }
    }
    ...
    if (awss_notify_resp[AWSS_NOTIFY_SUCCESS] == 0) {
        dump_awss_status(SUB_ERRCODE_WIFI_SENT_CONNECTAP_NOTI_TIMEOUT, "__awss_suc_notify %d timeout", suc_cnt);
    }

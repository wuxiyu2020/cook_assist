...
static void awss_report_reset_reply(void *pcontext, void *pclient, void *mesg)
{
    ...
    log_info(LOG_TAG, "got cloud reset reply");
...
int awss_report_reset_to_cloud(iotx_vendor_dev_reset_type_t *reset_type)
{
    ...
    if (awss_report_reset_suc) {
        dump_dev_bind_status(SUB_ERRCODE_BIND_ALREADY_RESET, "already reset");
        return SUB_ERRCODE_BIND_ALREADY_RESET;
    }
    ...
    do {
        ...
        if (topic == NULL) {
            ret = SUB_ERRCODE_SYS_DEPEND_MALLOC;
            dump_dev_bind_status(SUB_ERRCODE_BIND_REPORT_RESET_FAIL, "topic malloc fail");
            goto REPORT_RST_ERR;
        }
...
#endif
        if (ret < 0) {
            dump_dev_bind_status(SUB_ERRCODE_BIND_REPORT_RESET_FAIL, "awss reset reply sub fail");
            goto REPORT_RST_ERR;
        }
    ...
    if (packet == NULL) {
        ret = SUB_ERRCODE_SYS_DEPEND_MALLOC;
        dump_dev_bind_status(SUB_ERRCODE_BIND_REPORT_RESET_FAIL, "awss reset malloc fail");
        goto REPORT_RST_ERR;
    }
    ...
    log_debug(LOG_TAG, "reset_type=%d, report reset:%s\r\n", g_reset_type, packet);

    ret = IOT_MQTT_Publish_Simple(NULL, topic, IOTX_MQTT_QOS0, packet, final_len);
    log_debug(LOG_TAG, "report reset result:%d\r\n", ret);
    if (ret < 0) {
        dump_dev_bind_status(SUB_ERRCODE_BIND_REPORT_RESET_FAIL, topic);
    }
REPORT_RST_ERR:
...
int awss_report_reset(iotx_vendor_dev_reset_type_t *reset_type)
{
    ...
    if (ret < 0) {
        dump_dev_bind_status(SUB_ERRCODE_SYS_DEPEND_KV_SET, "set reset kv flag fail");
    }

    ret = HAL_Kv_Set(AWSS_KV_RST_TYPE, reset_type, sizeof(iotx_vendor_dev_reset_type_t), 0);
    if (ret < 0) {
        dump_dev_bind_status(SUB_ERRCODE_SYS_DEPEND_KV_SET, "set reset_type kv flag fail");
    }
...
int awss_check_reset(iotx_vendor_dev_reset_type_t *reset_type)
{
    ...
    {
        log_info(LOG_TAG, "has reset flag in kv");
    }
    ...
    if (ret != 0 || len == 0)
    {
        log_warning(LOG_TAG, "no rst type in kv");
        g_reset_type = IOTX_VENDOR_DEV_RESET_TYPE_UNBIND_SHADOW_CLEAR;
    }

    log_info(LOG_TAG, "need report rst,type=%d", g_reset_type);
    awss_report_reset_suc = 0;
...
int awss_reset_get_report_flag(void)
{
    int report_flag = 0;
    int len = 5;
    char reset_report_buff[5] = {0};

    int ret = HAL_Kv_Get("reset_report", reset_report_buff, &len);
    if (0 == ret)
    {
        if (!strncmp(reset_report_buff, "y", 1))
        {
            report_flag = 1;
        }
        else
    ...
    log_info(LOG_TAG, "reset report flag=%d", report_flag);
...
int awss_clear_reset(void)
{
    ...
    if (HAL_Kv_Del(AWSS_KV_RST) != 0) {
        log_err(LOG_TAG, "KV_RST del fail");
        ret = -1;
    }
    if (HAL_Kv_Del(AWSS_KV_RST_TYPE) != 0) {
        log_err(LOG_TAG, "KV_RST_TYPE del fail");
        ret = -1;
    }
...
{
    if (report_reset_timer == NULL) {
        dump_dev_bind_status(SUB_ERRCODE_BIND_ALREADY_RESET, "stop report already reset");
        return SUB_ERRCODE_BIND_ALREADY_RESET;
    }

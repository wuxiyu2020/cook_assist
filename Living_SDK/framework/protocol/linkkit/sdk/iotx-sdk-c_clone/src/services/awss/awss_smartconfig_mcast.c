...
{
    int passwd_len = 0;
    /* CAN'T use snawss_trace here, because of SPACE char */
    ...
    if (zconfig_is_utf8((const char *)zc_passwd, passwd_len) == 0) {
        dump_awss_status(SUB_ERRCODE_WIFI_PASSWD_DECODE_FAILED, "mcast passwd err");
...
int verify_checksum()
{
    ...
    ret = cal_crc - crc;
    dump_awss_status(STATE_CODE_WIFI_MCAST_DEBUG, "mcast: verify checksum diff is %d, cal_crc, crc is %d,%d", ret, cal_crc,
                     crc);
    return ret;
...
int parse_result()
{
    ...
    if (0x3 != BIT6_7_version) {
        dump_awss_status(SUB_ERRCODE_WIFI_UNEXP_PROT_VERSION, "mcast: error version");
        return SUB_ERRCODE_WIFI_UNEXP_PROT_VERSION;
    }
    ...
    if (SUCCESS_RETURN != ret) {
        dump_awss_status(SUB_ERRCODE_WIFI_CRC_ERROR, "mcast: checksum mismatch");
		reset_mcast_data();
        return SUB_ERRCODE_WIFI_CRC_ERROR;
    }
    ret = decode_passwd();
    gen16ByteToken();
    reset_mcast_data();
    if (SUCCESS_RETURN != ret) {
        dump_awss_status(SUB_ERRCODE_WIFI_PASSWD_DECODE_FAILED, "mcast: passwd error");
        return SUB_ERRCODE_WIFI_PASSWD_DECODE_FAILED;
    }
...
#endif //End AWSS_SUPPORT_SMARTCONFIG_MCAST
#if defined(__cplusplus)  /* If this is a C++ compiler, use C linkage */

...
int awss_clear_aplist(void)
{
    if (zconfig_aplist == NULL) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null to clear");
        return -1;
    }
...
int awss_init_ieee80211_aplist(void)
{
    ...
    if (zconfig_aplist == NULL) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null when init");
        return -1;
    }
    zconfig_aplist_num = 0;
    return 0;
...
struct ap_info *zconfig_get_apinfo(uint8_t *mac)
{
    int i;
    if (zconfig_aplist == NULL) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null when get");
        return NULL;
    }
...
struct ap_info *zconfig_get_apinfo_by_3_byte_mac(uint8_t *last_3_Byte_mac)
{
    ...
    uint8_t *local_mac;
    if (zconfig_aplist == NULL) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null when get 3B");
        return NULL;
    }
...
struct ap_info *zconfig_get_apinfo_by_ssid(uint8_t *ssid)
{
    int i;
    if (zconfig_aplist == NULL) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null to get by ssid");
        return NULL;
    }
...
struct ap_info *zconfig_get_apinfo_by_ssid_prefix(uint8_t *ssid_prefix)
{
    ...
    if (!len || (zconfig_aplist == NULL)) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null or ssid prefix null");
        return NULL;
    }
...
struct ap_info *zconfig_get_apinfo_by_ssid_suffix(uint8_t *ssid_suffix)
{
    ...
    if (!len || (zconfig_aplist == NULL)) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null or ssid suffix null");
        return NULL;
    }
                     ...
                     uint8_t pairwise_cipher, uint8_t group_cipher, signed char rssi)
{
    ...
    if (zconfig_aplist == NULL) {
        dump_awss_status(STATE_CODE_WIFI_STATISTIC, "aplist null to save");
        return -1;
    }
...
#endif
        dump_awss_status(STATE_CODE_WIFI_CHAN_SCAN, "[%d] ssid:%s, mac:%02x%02x%02x%02x%02x%02x, chn:%d, rssi:%d, adha:%d",

...
#define REPORT_TIME_INTERVAL  (60 * 60 * 1000)

static int iotx_mc_send_packet(iotx_mc_client_t *c, char *buf, int length, iotx_time_t *time);
static int iotx_mc_read_packet(iotx_mc_client_t *c, iotx_time_t *timer, unsigned int *packet_type, int timeout_ms);
...
#if WITH_MQTT_DYN_CONNINFO && !(WITH_MQTT_MULTI_INSTANCE)
void preauth_success_cb(void *client)
{
    iotx_mc_client_t *pClient = (iotx_mc_client_t *)client;
    if (pClient != NULL) {
        pClient->reconnect_param.reconnect_time_interval_ms = 0;
    }
}
#endif
...
static int _conn_info_dynamic_reload(iotx_mc_client_t *pClient)
{
    ...
    pconn = iotx_conn_info_reload(preauth_success_cb, (void*)pClient);
    if (pconn == NULL)
    {
        return NULL_VALUE_ERROR;
    }
    ...
    if (qos == IOTX_MQTT_QOS3_SUB_LOCAL) {
        ...
        for (h = c->first_sub_handle; h; h = h->next) {
            ...
            if (0 == iotx_mc_check_handle_is_identical(h, handler)) {
                mqtt_info("dup sub,topic = %s", topicFilter);
                dup = 1;
            }
        }
        if (dup == 0) {
            handler->qos = IOTX_MQTT_QOS3_SUB_LOCAL;
...
//Not unsub to cloud if this is cloud sub topic
#if (WITH_MQTT_SUB_LOCAL)
    do
    {
        iotx_mc_topic_handle_t *h;
        ...
        HAL_MutexLock(c->lock_generic);
        mqtt_info("unsubscribe topic(%s)", topicFilter);
        for (h = c->first_sub_handle; h; h = h->next)
        {
            if (h->qos == IOTX_MQTT_QOS3_SUB_LOCAL) //this is cloud sub topic
            {
#if !(WITH_MQTT_ZIP_TOPIC)
                if (0 == strncmp(h->topic_filter, handler->topic_filter, strlen(handler->topic_filter)))
#else
                if ( (handler->topic_type == TOPIC_FILTER_TYPE && 0 == strncmp(h->topic_filter, handler->topic_filter, strlen(handler->topic_filter)))
                    || (handler->topic_type == TOPIC_NAME_TYPE && (0 == memcmp(h->topic_filter, handler->topic_filter, MQTT_MD5_PATH_DEFAULT_LEN))) )
#endif
                {
                    mqtt_info("not unsub to cloud just release topic(%s)", topicFilter);
                    mqtt_free(handler->topic_filter);
                    mqtt_free(handler);
...
#endif
                {
                    mqtt_debug("unsub cloud topic:%s", topicFilter);
                    break;
                }
            }
            if (h->next == NULL) {
                mqtt_debug("NOT matched topic:%s", topicFilter);
                mqtt_free(handler->topic_filter);
                mqtt_free(handler);
                HAL_MutexUnlock(c->lock_generic);
                return SUCCESS_RETURN;
...
static int iotx_mc_handle_recv_PUBREC(iotx_mc_client_t *c)
{
    unsigned short mypacketid;
    unsigned char  dup  = 0;
    unsigned char  type = 0;
    ...
    if (MQTTDeserialize_ack(&type, &dup, &mypacketid, (unsigned char *)c->buf_read, c->buf_size_read) != 1) {
        mqtt_err("MQTTDeserialize_ack failed");
    ...
    if ((len = MQTTSerialize_ack((unsigned char *)c->buf_send, c->buf_size_send, PUBREL, 0, mypacketid)) <= 0) {
        mqtt_err("MQTTSerialize_ack failed");
        _reset_send_buffer(c);
        HAL_MutexUnlock(c->lock_write_buf);
        return MQTT_PUBLISH_ACK_PACKET_ERROR;
    }
    if (iotx_mc_send_packet(c, c->buf_send, len, &timer) != SUCCESS_RETURN) {  // send the PUBREL packet
        mqtt_err("iotx_mc_send_packet failed");
        _reset_send_buffer(c);
        HAL_MutexUnlock(c->lock_write_buf);
        return FAIL_RETURN;
    }
...
static int iotx_mc_handle_recv_PUBREL(iotx_mc_client_t *c)
{
    unsigned short mypacketid;
    unsigned char  dup  = 0;
    unsigned char  type = 0;
    ...
    if (MQTTDeserialize_ack(&type, &dup, &mypacketid, (unsigned char *)c->buf_read, c->buf_size_read) != 1) {
        mqtt_err("MQTTDeserialize_ack failed");
        return MQTT_PUBLISH_REL_PACKET_ERROR;
    }
    HAL_MutexLock(c->lock_write_buf);
    if (_alloc_send_buffer(c, 0) < 0) {
        HAL_MutexUnlock(c->lock_write_buf);
    ...
    if ((len = MQTTSerialize_ack((unsigned char *)c->buf_send, c->buf_size_send, PUBCOMP, 0, mypacketid)) <= 0) {
        mqtt_err("MQTTSerialize_ack failed");
        _reset_send_buffer(c);
        HAL_MutexUnlock(c->lock_write_buf);
        return MQTT_PUBLISH_ACK_PACKET_ERROR;
...
static int iotx_mc_handle_recv_PUBCOMP(iotx_mc_client_t *c)
{
    unsigned short mypacketid;
    unsigned char  dup  = 0;
    unsigned char  type = 0;
...
#define WAIT_CONNACK_MAX (10)
    ...
    do {
        ...
        rc = iotx_mc_read_packet(c, &timer, &packetType, c->request_timeout_ms);
        if (rc != SUCCESS_RETURN) {
...
static int iotx_mc_cycle(iotx_mc_client_t *c, iotx_time_t *timer, int timeout_ms)
{
    ...
    if (state != IOTX_MC_STATE_CONNECTED) {
        mqtt_debug("iotx_mc_cycle state = %d", state);
    ...
    /* read the socket, see what work is due */
    rc = iotx_mc_read_packet(c, timer, &packetType, timeout_ms);
    if (rc != SUCCESS_RETURN) {
        HAL_MutexLock(c->lock_read_buf);
        _reset_recv_buffer(c);
        HAL_MutexUnlock(c->lock_read_buf);
    ...
    switch (packetType) {
        ...
        case PUBREC: {
            mqtt_debug("PUBREC");
            rc = iotx_mc_handle_recv_PUBREC(c);
            if (SUCCESS_RETURN != rc) {
                mqtt_err("recvPubreckProc error,result = %d", rc);
            }
            break;
        }
        case PUBREL: {
            mqtt_debug("PUBREL");
            rc = iotx_mc_handle_recv_PUBREL(c);
            if (SUCCESS_RETURN != rc) {
                mqtt_err("recvPubrelProc error,result = %d", rc);
            }
            break;
        }
        case PUBCOMP: {
            mqtt_debug("PUBCOMP");
            rc = iotx_mc_handle_recv_PUBCOMP(c);
            if (SUCCESS_RETURN != rc) {
                mqtt_err("recvPubcompProc error,result = %d", rc);
            }
            break;
        }
#endif
...
int iotx_mc_publish(iotx_mc_client_t *c, const char *topicName, iotx_mqtt_topic_info_pt topic_msg)
{
    ...
    mqtt_info("mqtt publish mesgid:%d ok", msg_id);
...
static void iotx_mc_keepalive(iotx_mc_client_t *pClient)
{
    ...
    do {
        ...
        if (IOTX_MC_STATE_DISCONNECTED_RECONNECTING == currentState) {
            ...
            if (SUCCESS_RETURN == rc) {
                mqtt_err("network is reconnected!");
...
int iotx_mc_connect(iotx_mc_client_t *pClient)
{
    ...
    if (SUCCESS_RETURN != rc) {
        ...
        if (ERROR_CERTIFICATE_EXPIRED == rc) {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CERT_VERIFY_FAIL, "certificate is expired!");
            return ERROR_CERT_VERIFY_FAIL;
        } else {
            dump_mqtt_status(SUB_ERRCODE_MQTT_NETWORK_CONNECT_ERROR, "network connect error");
    ...
    if (rc  != SUCCESS_RETURN) {
        pClient->ipstack->disconnect(pClient->ipstack);
        dump_mqtt_status(SUB_ERRCODE_MQTT_CONNECT_PKT_SEND_FAIL, "send connect packet failed");
        return  rc;
    }
    ...
    if (SUCCESS_RETURN != rc) {
        ...
        if (rc == MQTT_CONANCK_UNACCEPTABLE_PROTOCOL_VERSION_ERROR) {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CONNACK_VERSION_UNACCEPT, "mqtt ack version unaccept");
        } else if (rc == MQTT_CONNACK_IDENTIFIER_REJECTED_ERROR) {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CONNACK_IDENT_REJECT, "mqtt ack ident reject");
        } else if (rc == MQTT_CONNACK_SERVER_UNAVAILABLE_ERROR) {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CONNACK_SERVICE_NA, "mqtt ack service na");
        } else if (rc == MQTT_CONNACK_BAD_USERDATA_ERROR) {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CONNACK_BAD_USERDATA, "mqtt ack bad userdata");
        } else if (rc == MQTT_CONNACK_NOT_AUTHORIZED_ERROR) {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CONNACK_NOT_AUTHORIZED, "mqtt ack not authorized");
        } else {
            dump_mqtt_status(SUB_ERRCODE_MQTT_CONNECT_UNKNOWN_FAIL, "mqtt ack unknown fail");
        }
...
int iotx_mc_attempt_reconnect(iotx_mc_client_t *pClient)
{
    ...
    now_time = HAL_UptimeMs();
    if (now_time - pClient->pre_report_timestamp > REPORT_TIME_INTERVAL) {
        iotx_report_connect_status(pClient, pClient->conn_cost);
        pClient->pre_report_timestamp = now_time;
    }
    return SUCCESS_RETURN;
}
...
int iotx_mc_handle_reconnect(iotx_mc_client_t *pClient)
{
    ...
    if (IOTX_MC_RECONNECT_INTERVAL_MAX_MS < interval_ms) {
        interval_ms = IOTX_MC_RECONNECT_INTERVAL_MAX_MS + HAL_Random(IOTX_MC_RECONNECT_INTERVAL_MAX_MS);
    }
...
void *IOT_MQTT_Construct(iotx_mqtt_param_t *pInitParams)
{
    ...
    if (NULL == pclient) {
        ...
        _conn_info_dynamic_release();
        dump_mqtt_status(SUB_ERRCODE_MQTT_INIT_FAIL, "not enough memory.");
        return NULL;
    }
    ...
    err = iotx_reconnect_region_subscribe();
    if (err < 0) {
...
int IOT_MQTT_Yield(void *handle, int timeout_ms)
{
    ...
    do {
        ...
        rc = iotx_mc_cycle(pClient, &time, timeout_ms);
        if (SUCCESS_RETURN == rc) {
#if !WITH_MQTT_ONLY_QOS0
...
int IOT_MQTT_Publish_Simple(void *handle, const char *topic_name, int qos, void *data, int len)
{
    ...
    if (rc < 0) {
        mqtt_err("mqtt publish fail:%d", rc);
        return -1;
    }

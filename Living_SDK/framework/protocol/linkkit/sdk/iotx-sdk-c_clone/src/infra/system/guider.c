...
const char *domain_mqtt_direct_pre[] = {
    "iot-test-pre.iot-as-mqtt.unify.aliyuncs.com",    /* Shanghai */
    "iot-test-sg-pre.iot-as-mqtt.unify.aliyuncs.com",    /* Singapore */
    "pre.Japan",        /* Japan */
    "pre.America.east", /* America east*/
    "iot-test-de-pre.iot-as-mqtt.unify.aliyuncs.com",   /* Germany */
...
int iotx_guider_set_dynamic_region(int region)
{
    ...
    if (GUIDER_ENV_PRERELEASE == iotx_guider_get_env())
    {
        ...
        if (!strcmp("iot-test-pre.iot-as-mqtt.unify.aliyuncs.com", domain_mqtt_direct_pre[region]))
        {
            port = 1883;
        }
                                ...
                                char *p_guider_sign, secure_mode_e secure_mode, char *p_dynamic_mqtt_url)
{
    ...
    {
        reset_type = IOTX_VENDOR_DEV_RESET_TYPE_INVALID;
    }
#endif
...
int iotx_guider_authenticate(iotx_conn_info_t *conn_info, void (*cb)(void *), void *client)
{
    ...
    if (CONNECT_DIRECT == bootup_connect_method)
    {
        ...
        if (strlen(p_dynamic_mqtt_url) > 0)
        {
            ...
            sys_crit("dyna url:%s count:%d", p_dynamic_mqtt_url, direct_connect_count);

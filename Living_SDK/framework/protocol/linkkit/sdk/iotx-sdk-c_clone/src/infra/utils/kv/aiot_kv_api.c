...
{
    int ret = aiot_kv_set(key, value, len, sync);

#ifdef CONFIG_WIRELESS_LOG
    if (ret != RES_OK)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_KV_SET, "kv set key(%s) fail:%d", key, ret);
    }
#endif
...
{
    int ret = aiot_kv_get(key, buffer, buffer_len);

#ifdef CONFIG_WIRELESS_LOG
    if (ret != RES_OK && ret != RES_ITEM_NOT_FOUND)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_KV_GET, "kv get key(%s) fail:%d", key, ret);
    }
#endif
...
{
    int ret = aiot_kv_del(key);

#ifdef CONFIG_WIRELESS_LOG
    if (ret != RES_OK && ret != RES_ITEM_NOT_FOUND)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_KV_DELETE, "kv del key(%s) fail:%d", key, ret);
    }
#endif
...
{
    int ret = aiot_kv_del_by_prefix(prefix);

#ifdef CONFIG_WIRELESS_LOG
    if (ret != RES_OK)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_KV_DELETE, "kv del by prefix(%s) fail:%d", prefix, ret);
    }
#endif

                                   ...
                                   _OU_ char device_secret[DEVICE_SECRET_MAXLEN])
{
    ...
    domain = iotx_guider_get_domain(GUIDER_DOMAIN_DYNAMIC_REGISTER_HTTP);
    if (NULL == domain)
    {
    ...
    if (res != SUCCESS_RETURN)
    {
        ...
        dump_user_input_status(SUB_ERRCODE_USER_INPUT_HTTP_DOMAIN, "http url:%s request fail:%d", url, res);
#endif

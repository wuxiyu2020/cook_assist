...
#define IOTX_LINKKIT_ASYNC_DEFAULT_TIMEOUT_MS 60000 //Change max timeout to 60S
...
static int  cloud_connect_state = 0;
int get_cloud_connect_state(void)
{
    return cloud_connect_state;
}
static void _iotx_linkkit_event_callback(iotx_dm_event_types_t type, char *payload)
{
    ...
    switch (type) {
        case IOTX_DM_EVENT_CLOUD_CONNECTED: {
            ...
            extern void timer_ntp_sync_post(void);
            timer_ntp_sync_post();
#endif
        }
        break;
        case IOTX_DM_EVENT_CLOUD_DISCONNECT: {
            cloud_connect_state = 0;
	    ...
	    extern int iotx_alcs_localgroup_rsp(const char *payload, int payload_len, int msg_from);
            if ( NULL != strstr(property_payload, "_LivingLink.alcs.localgroup") ){
                iotx_alcs_localgroup_rsp(property_payload, lite_item_payload.value_length, 2);
            ...
            if ((lite_identifier.type == cJSON_String) && !strncmp(lite_identifier.value, "awss.BindNotify", strlen("awss.BindNotify")) &&
                lite_iden_val.type == cJSON_Object) {
                lite_cjson_t lite_operation;

                sdk_info("awss.BindNotify");
                dm_utils_json_object_item(&lite_iden_val, "Operation", strlen("Operation"), cJSON_Invalid, &lite_operation);

                if (lite_operation.type == cJSON_String) {
                    if (!strncmp(lite_operation.value, "Bind", strlen("Bind"))) {
                        sdk_info("awss.BindNotify.Bind");
                        char *bind_flag = "y";
                        HAL_Kv_Set("device.bind", bind_flag, strlen(bind_flag) + 1, 0);
                        breeze_start_advertising(1, 0);
                    } else if (!strncmp(lite_operation.value, "Unbind", strlen("Unbind"))) {
                        sdk_info("awss.BindNotify.UnBind");
	    ...
	    extern int iotx_alcs_localgroup_rsp(const char *payload, int payload_len, int msg_from);
            ...
            {
                iotx_alcs_localgroup_rsp(user_payload, lite_item_payload.value_length, 1);
                is_need_callback = 0;
            }
#endif
            if (is_need_callback)

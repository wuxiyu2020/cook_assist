...
void *HAL_MutexCreate(void)
{
    ...
    if (0 != ret)
    {
        ...
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_MUTEX_CREATE, "create mutex fail:%d", ret);
#endif
        return NULL;
    }
...
void HAL_MutexLock(_IN_ void *mutex)
{
    ...
    if (NULL != mutex)
    {
        ret = aos_mutex_lock((aos_mutex_t *)&mutex, AOS_WAIT_FOREVER);
    }

#ifdef CONFIG_WIRELESS_LOG
    if (ret != 0)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_MUTEX_LOCK, "mutex lock fail:%d", ret);
    }
#endif
}
...
void HAL_MutexUnlock(_IN_ void *mutex)
{
    ...
    if (NULL != mutex)
    {
        ret = aos_mutex_unlock((aos_mutex_t *)&mutex);
    }

#ifdef CONFIG_WIRELESS_LOG
    if (ret != 0)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_MUTEX_UNLOCK, "mutex unlock fail:%d", ret);
    }
#endif
}
...
{
    void *p_mem = aos_malloc(size);

#ifdef CONFIG_WIRELESS_LOG
    if (p_mem == NULL)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_MALLOC, "malloc size:%u fail", size);
    }
#endif
...
{
    void *p_mem = aos_realloc(ptr, size);

#ifdef CONFIG_WIRELESS_LOG
    if (p_mem == NULL)
    {
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_MALLOC, "realloc size:%u fail", size);
    }
#endif
...
void HAL_Printf(_IN_ const char *fmt, ...)
{
    ...
    if (0 == log_get_enable_aos_log_flag())
        return;
#endif

    if (LITE_get_loglevel() == AOS_LL_V_NONE)
        return;
                     ...
                     _OU_ int *stack_used)
{
    ...
    if (aos_task_new_ext(&task->task, tname, task_wrapper, task, ssiz, priority) == 0)
    {
...
FAIL:
#ifdef CONFIG_WIRELESS_LOG
    dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_THREAD_CREATE, "create thread:%s fail", tname);
#endif
...
void *HAL_Timer_Create(const char *name, void (*func)(void *), void *user_data)
{
    ...
    {
#ifdef CONFIG_WIRELESS_LOG
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_TIMER_CREATE, "create timer no mem");
#endif
        return NULL;
    }
...
int HAL_Timer_Start(void *t, int ms)
{
    if (t != NULL && ms >= 0)
    {
        ...
        timer->ms = ms;
        aos_schedule_call(schedule_timer, t);
    ...
    {
#ifdef CONFIG_WIRELESS_LOG
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_TIMER_START, "start timer fail");
#endif
    }
...
int HAL_Timer_Stop(void *t)
{
    if (t != NULL)
    {
        aos_schedule_call(schedule_timer_cancel, t);
    ...
    {
#ifdef CONFIG_WIRELESS_LOG
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_TIMER_STOP, "stop timer fail");
#endif
    }
...
int HAL_Timer_Delete(void *timer)
{
    if (timer != NULL)
    {
        aos_schedule_call(schedule_timer_delete, timer);
        return 0;
    }
    else
    {
#ifdef CONFIG_WIRELESS_LOG
        dump_sys_depend_status(SUB_ERRCODE_SYS_DEPEND_TIMER_DELETE, "del timer fail");
#endif
    }

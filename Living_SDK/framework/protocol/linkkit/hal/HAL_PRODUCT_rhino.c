...
int HAL_GetProductKey(_OU_ char *product_key)
{
    ...
    if (len < 1 || len > PRODUCT_KEY_LEN)
    {
        dump_user_input_status(SUB_ERRCODE_USER_INPUT_PK, "pk invalid");
    }
#endif
...
#ifdef CONFIG_WIRELESS_LOG
    if (len < 1 || len > PRODUCT_SECRET_LEN)
    {
        dump_user_input_status(SUB_ERRCODE_USER_INPUT_PS, "ps invalid");
    }
#endif
...
#ifdef CONFIG_WIRELESS_LOG
    if (len < 1 || len > DEVICE_NAME_LEN)
    {
        dump_user_input_status(SUB_ERRCODE_USER_INPUT_DN, "dn invalid");
    }
#endif
...
#ifdef CONFIG_WIRELESS_LOG
    if (len < 1 || len > DEVICE_SECRET_LEN)
    {
        dump_user_input_status(SUB_ERRCODE_USER_INPUT_DS, "ds invalid");
    }
#endif
